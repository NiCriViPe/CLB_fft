...
##  Matplotlib.pyplot
from matplotlib.pyplot import plot,subplots,xlim,ylim,xlabel,ylabel,grid,minorticks_on,tight_layout
from matplotlib.pyplot import show,subplots_adjust,vlines,text,semilogy,subplot2grid,legend,title
from matplotlib.pyplot import rcParams,locator_params,gca,axes
##  Time
import time
##  OS
import os
...
def ejecutar(v):
    ...
    return
def PSD(v):
    if  (os.path.isfile(v.replace(".csv","_y-t_DATA.csv"))) == False:
        ejecutar(v)
...
def gen_lista_comparar(n):
    ...
    for i in range(n):
        ...
        v = filedialog.askopenfilename(initialdir = OUTPUT_PATH,title = "Select file",filetypes = (("csv files","*.csv"),("all files","*.*")))
